...
class Testcases(unittest.TestCase):
    ...
    def test_vote(self):
        op = operations.Vote(**{"voter": "foobara", "author": "foobarc", "permlink": "foobard", "weight": 1000})
        ops = [operations.Operation(op)]
        tx = SignedTransaction(
            ref_block_num=ref_block_num, ref_block_prefix=ref_block_prefix, expiration=expiration, operations=ops
        )
    ...
    def test_transfer(self):
        op = operations.Transfer(**{"from": "foo", "to": "baar", "amount": "111.110 STEEM", "memo": "Fooo"})
        ops = [operations.Operation(op)]
        tx = SignedTransaction(
            ref_block_num=ref_block_num, ref_block_prefix=ref_block_prefix, expiration=expiration, operations=ops
        )
    ...
    def test_change_recovery_account(self):
        op = operations.ChangeRecoveryAccount(
            **{"account_to_recover": "barrie", "extensions": [], "new_recovery_account": "boombastic"}
        )
        ops = [operations.Operation(op)]
        tx = SignedTransaction(
            ref_block_num=ref_block_num, ref_block_prefix=ref_block_prefix, expiration=expiration, operations=ops
    ...
    def test_set_route(self):
        op = operations.SetWithdrawVestingRoute(
            **{"from_account": "xeroc", "to_account": "xeroc", "percent": 1000, "auto_vest": False}
        )
        ops = [operations.Operation(op)]
        tx = SignedTransaction(
            ref_block_num=ref_block_num, ref_block_prefix=ref_block_prefix, expiration=expiration, operations=ops
    ...
    def test_convert(self):
        op = operations.Convert(**{"owner": "xeroc", "requestid": 2342343235, "amount": "100.000 SBD"})
        ops = [operations.Operation(op)]
        tx = SignedTransaction(
            ref_block_num=ref_block_num, ref_block_prefix=ref_block_prefix, expiration=expiration, operations=ops
        )
    ...
    def test_feed_publish(self):
        op = operations.FeedPublish(
            **{"publisher": "xeroc", "exchange_rate": {"base": "1.000 SBD", "quote": "4.123 STEEM"}}
        )
        ops = [operations.Operation(op)]
        tx = SignedTransaction(
            ref_block_num=ref_block_num, ref_block_prefix=ref_block_prefix, expiration=expiration, operations=ops
    ...
    def test_witness_set_properties(self):
        ...
        tx_wire = hexlify(compat_bytes(tx)).decode("ascii")
        compare = (
            "f68585abf4dce7c80457012a06696e69742d3102146163636f75"
    ...
    def test_comment_options(self):
        op = operations.CommentOptions(
            **{
                ...
                "beneficiaries": [{"weight": 2000, "account": "good-karma"}, {"weight": 5000, "account": "null"}],
            }
        )
        ops = [operations.Operation(op)]
        tx = SignedTransaction(
    ...
    def compare_constructed_tx(self):
        ...
        compare = rpc.serialize_transaction(tx.json())  # type: ignore[attr-defined]

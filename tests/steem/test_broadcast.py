...
def test_claim_reward():
    ...
    try:
        c.claim_reward_balance(
            account='test', reward_steem='1.000 STEEM', reward_vests='0.000000 VESTS', reward_sbd='0.000 SBD'
        )
    except RPCError as e:
        rpc_error = str(e)
    else:
...
def test_witness_update():
    ...
    props = {'account_creation_fee': '0.500 STEEM', 'maximum_block_size': 65536, 'sbd_interest_rate': 0}

    rpc_error = None
    try:
        c.witness_update(signing_key=signing_key, account='test', props=props, url='foo')
    except RPCError as e:
        rpc_error = str(e)
    else:
        raise Exception('expected RPCError')
...
def test_witness_set_properties():
    ...
    rpc_error = None
    try:
        c.witness_set_properties(signing_key=signing_key, props=props, account='test')
    except RPCError as e:
        rpc_error = str(e)
    else:
        raise Exception('expected RPCError')

...
class DataDir(object):
    ...
    def sqlite3_backup(self, dbfile, backupdir):
        ...
        backup_file = os.path.join(
            backupdir, os.path.basename(self.storage_database) + datetime.now().strftime("-" + timeformat)
        )
...
class Key(DataDir):
    ...
    def exists_table(self):
        ...
        query = ("SELECT name FROM sqlite_master " + "WHERE type='table' AND name=?", (self.__tablename__,))
        connection = sqlite3.connect(self.sql_database_file)
        cursor = connection.cursor()
    ...
    def update_wif(self, pub, wif):
        ...
        """
        query = ("UPDATE %s " % self.__tablename__ + "SET wif=? WHERE pub=?", (wif, pub))
        connection = sqlite3.connect(self.sql_database_file)
        cursor = connection.cursor()
        cursor.execute(*query)
        connection.commit()
    ...
    def add(self, wif, pub):
        ...
        query = ('INSERT INTO %s (pub, wif) ' % self.__tablename__ + 'VALUES (?, ?)', (pub, wif))
        connection = sqlite3.connect(self.sql_database_file)
        cursor = connection.cursor()
        cursor.execute(*query)
        connection.commit()
...
class Configuration(DataDir):
    ...
    def check_backup(self):
        ...
        if "lastBackup" not in config_storage or config_storage["lastBackup"] == "":
            print("No backup has been created yet!")
            self.refresh_backup()
        try:
            if (datetime.now() - datetime.strptime(config_storage["lastBackup"], timeformat)).days > 7:
    ...
    def __setitem__(self, key, value):
        if self._have_key(key):
            query = ("UPDATE %s " % self.__tablename__ + "SET value=? WHERE key=?", (value, key))
        else:
            query = ("INSERT INTO %s " % self.__tablename__ + "(key, value) VALUES (?, ?)", (key, value))
        connection = sqlite3.connect(self.sql_database_file)
        cursor = connection.cursor()
        cursor.execute(*query)
        connection.commit()
...
class KeyEncryptionKey(object):
    ...
    def save_encrytped_kek(self):
        """Store the encrypted KeyEncryptionKey in the configuration store."""
    ...
    def get_encrypted_kek(self):
        ...
        aes = AESCipher(self.user_passphrase)
        return "{}${}".format(self.derive_checksum(self.decrypted_kek), aes.encrypt(self.decrypted_kek))

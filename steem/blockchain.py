...
class Blockchain(object):
    ...
    def reliable_stream(
        ...
        **kwargs,
        ...
        def reliable_query(_client, _method, _api, *_args):
            ...
            while True:
                try:
                    ...
                    logger.error(
                        'Error: %s' % str(e),
                        extra={"exc": e, "response": None, "api_name": _api, "api_method": _method, "api_args": _args},
                    )
                    time.sleep(1)

        def get_reliable_block_interval(_client):
            return reliable_query(_client, 'get_config', 'database_api').get('STEEM_BLOCK_INTERVAL')
        ...
        def get_reliable_blockdata(_client, _block_num):
            return reliable_query(_client, 'get_block', 'database_api', _block_num)

        def get_reliable_ops_in_block(_client, _block_num):
            return reliable_query(_client, 'get_ops_in_block', 'database_api', _block_num, False)
        ...
        while True:
            ...
            for block_num in range(start_block, head_block + 1):
                if full_blocks:
                    yield get_reliable_blockdata(_reliable_client, block_num)
                elif batch_operations:
                    yield get_reliable_ops_in_block(_reliable_client, block_num)
                else:
                    for reliable_ops in get_reliable_ops_in_block(_reliable_client, block_num):
                        yield reliable_ops
    ...
    def history(self, filter_by=None, start_block=1, end_block=None, raw_output=False, **kwargs):
    ...
    def replay(self, **kwargs):
        warnings.warn('Blockchain.replay() is deprecated. ' + 'Please use Blockchain.history() instead.', stacklevel=2)
    ...
    def get_all_usernames(self, *args, **kwargs):
        ...
        _ = args, kwargs
        warnings.warn('Blockchain.get_all_usernames() is now Steemd.get_all_usernames().', stacklevel=2)

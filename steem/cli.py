...
def legacyentry():
    ...
    parser.add_argument('--no-broadcast', '-d', action='store_true', help='Do not broadcast anything')
    parser.add_argument('--no-wallet', '-p', action='store_true', help='Do not load the wallet')
    parser.add_argument('--unsigned', '-x', action='store_true', help='Do not try to sign the transaction')
    parser.add_argument('--expires', '-e', default=60, help='Expiration time in seconds (defaults to 30)')
    parser.add_argument('--verbose', '-v', type=int, default=3, help='Verbosity')
    parser.add_argument('--version', action='version', version='%(prog)s {version}'.format(version=version("steempy")))
    ...
    setconfig.add_argument('key', type=str, choices=available_configuration_keys, help='Configuration key')
    ...
    addkey.add_argument(
        ...
        nargs='*',
        type=str,
        help='private key to import into the wallet (unsafe, unless you ' + 'delete your shell history)',
    )
    ...
    parsewif.add_argument(
        '--unsafe-import-key', nargs='*', type=str, help='WIF key to parse (unsafe, delete your bash history)'
    )
    ...
    delkey.add_argument('pub', nargs='*', type=str, help='the public key to delete from the wallet')
    ...
    getkey.add_argument('pub', type=str, help='the public key for which to show the private key')
    ...
    parser_upvote.add_argument(
        'post', type=str, help='author/permlink-identifier of the post to upvote ' + 'to (e.g. xeroc/python-steem-0-1)'
    )
    parser_upvote.add_argument(
        '--account', type=str, required=False, default=config_storage["default_account"], help='The voter account name'
    )
    parser_upvote.add_argument(
        '--weight',
        type=float,
    ...
    parser_downvote.add_argument(
        '--account', type=str, default=config_storage["default_account"], help='The voter account name'
    )
    parser_downvote.add_argument(
        'post',
        type=str,
    ...
    parser_transfer.add_argument(
        'asset', type=str, choices=["STEEM", "SBD"], help='Asset to transfer (i.e. STEEM or SDB)'
    )
    parser_transfer.add_argument('memo', type=str, nargs="?", default="", help='Optional memo')
    parser_transfer.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    parser_powerup.add_argument('--to', type=str, required=False, default=None, help='Powerup this account')
    ...
    parser_powerdown.add_argument('amount', type=str, help='Amount of VESTS to powerdown')
    ...
    parser_powerdownroute.add_argument(
        '--percentage', type=float, default=100, help='The percent of the withdraw to go to the "to" account'
    )
    parser_powerdownroute.add_argument(
        '--account', type=str, default=config_storage["default_account"], help='The account which is powering down'
    )
    parser_powerdownroute.add_argument(
        '--auto_vest',
        action='store_true',
    ...
    parser_convert.add_argument('amount', type=float, help='Amount of SBD to convert')
    ...
    interest.add_argument(
        'account', type=str, nargs="*", default=config_storage["default_account"], help='Inspect these accounts'
    )
    ...
    parser_allow.add_argument(
        ...
        type=str,
        nargs="?",
        help='The account or key that will be allowed to interact with ' + 'your account',
    )
    parser_allow.add_argument(
        '--permission',
        type=str,
    ...
    parser_newaccount.add_argument(
        '--fee', type=str, required=False, default='0 STEEM', help='Base Fee to pay. Delegate the rest.'
    )
    ...
    parser_updatememokey.add_argument('--key', type=str, default=None, help='The new memo key')
    ...
    parser_approvewitness.add_argument(
        '--account', type=str, required=False, default=config_storage["default_account"], help='Your account'
    )
    ...
    parser_disapprovewitness.add_argument('witness', type=str, help='Witness to disapprove')
    ...
    orderbook.add_argument('--chart', action='store_true', help="Enable charting (requires matplotlib)")
    ...
    parser_buy.add_argument('asset', type=str, choices=["STEEM", "SBD"], help='Asset to buy (i.e. STEEM or SDB)')
    parser_buy.add_argument('price', type=float, help='Limit buy price denoted in (SBD per STEEM)')
    ...
    parser_sell.add_argument('asset', type=str, choices=["STEEM", "SBD"], help='Asset to sell (i.e. STEEM or SDB)')
    parser_sell.add_argument('price', type=float, help='Limit sell price denoted in (SBD per STEEM)')
    ...
    parser_resteem.add_argument('identifier', type=str, help='author/permlink-identifier of the post to resteem')
    ...
    parser_unfollow.add_argument(
        '--what', type=str, required=False, nargs="*", default=[], help='Unfollow these objects (defaults to "blog")'
    )
    ...
    parser_setprofile_a.add_argument('--pair', type=str, nargs='*', help='"Key=Value" pairs')
    parser_setprofile_b = parser_setprofile.add_argument_group('Just a single key')
    parser_setprofile_b.add_argument('variable', type=str, nargs='?', help='Variable to set')
    ...
    parser_delprofile.add_argument('variable', type=str, nargs='*', help='Variable to set')
    ...
    parser_witnessprops.add_argument(
        '--witness', type=str, default=config_storage["default_account"], help='Witness name'
    )
    parser_witnessprops.add_argument('--maximum_block_size', type=float, required=False, help='Max block size')
    parser_witnessprops.add_argument('--account_creation_fee', type=float, required=False, help='Account creation fee')
    parser_witnessprops.add_argument(
        '--sbd_interest_rate', type=float, required=False, help='SBD interest rate in percent'
    )
    parser_witnessprops.add_argument('--url', type=str, required=False, help='Witness URL')
    parser_witnessprops.add_argument('--signing_key', type=str, required=False, help='Signing Key')
    ...
    parser_witnesscreate.add_argument('--maximum_block_size', type=float, default="65536", help='Max block size')
    parser_witnesscreate.add_argument('--account_creation_fee', type=float, default=30, help='Account creation fee')
    parser_witnesscreate.add_argument(
        '--sbd_interest_rate', type=float, default=0.0, help='SBD interest rate in percent'
    )
    ...
    verbosity = ["critical", "error", "warn", "info", "debug"][int(min(args.verbose, 4))]
    ...
    # GrapheneAPI logging
    if args.verbose > 4:
        verbosity = ["critical", "error", "warn", "info", "debug"][int(min((args.verbose - 4), 4))]
        gphlog = logging.getLogger("graphenebase")
        gphlog.setLevel(getattr(logging, verbosity.upper()))
        gphlog.addHandler(ch)
    if args.verbose > 8:
        verbosity = ["critical", "error", "warn", "info", "debug"][int(min((args.verbose - 8), 4))]
    ...
    if args.command == "set":
        ...
        if not args.objects:
            ...
            steem_per_mvest = Amount(info["total_vesting_fund_steem"]).amount / (
                Amount(info["total_vesting_shares"]).amount / 1e6
            )
            price = Amount(median_price["base"]).amount / Amount(median_price["quote"]).amount
            for key in info:
            ...
            # Account name
                ...
                for key in sorted(account):
                    ...
                    if key == "reputation" and int(value) > 0:
                        value = int(value)
                        rep = max(log10(value) - 9, 0) * 9 + 25 if value > 0 else max(log10(-value) - 9, 0) * -9 + 25
        ...
        if args.unsafe_import_key:
            ...
            while True:
                ...
                if len(installed_keys) == 1:
                    name = steem.commit.wallet.get_account_from_public_key(installed_keys[0])
                    print("=" * 30)
        ...
        print_json(steem.commit.transfer(args.to, args.amount, args.asset, memo=args.memo, account=args.account))

    elif args.command == "powerup":
        print_json(steem.commit.transfer_to_vesting(args.amount, account=args.account, to=args.to))
        ...
        print_json(
            steem.commit.set_withdraw_vesting_route(
                args.to, percentage=args.percentage, account=args.account, auto_vest=args.auto_vest
            )
        )
        ...
        t = PrettyTable(["Account", "Last Interest Payment", "Next Payment", "Interest rate", "Interest"])
        t.align = "r"
        ...
        print_json(
            steem.commit.disallow(
                args.foreign_account, account=args.account, permission=args.permission, threshold=args.threshold
            )
        )
        ...
        print_json(steem.commit.update_memo_key(args.key, account=args.account))
        ...
        print_json(steem.commit.approve_witness(args.witness, account=args.account))

    elif args.command == "disapprovewitness":
        print_json(steem.commit.disapprove_witness(args.witness, account=args.account))

    elif args.command == "resteem":
        print_json(steem.commit.resteem(args.identifier, account=args.account))

    elif args.command == "follow":
        print_json(steem.commit.follow(args.follow, what=args.what, account=args.account))

    elif args.command == "unfollow":
        print_json(steem.commit.unfollow(args.unfollow, what=args.what, account=args.account))
        ...
        print_json(steem.commit.update_account_profile(account["json_metadata"], account=args.account))
        ...
        print_json(
            steem.commit.witness_update(
                args.signing_key or witness["signing_key"], args.url or witness["url"], props, account=args.witness
            )
        )
        ...
        print_json(steem.commit.witness_update(args.signing_key, args.url, props, account=args.witness))
...
def format_operation_details(op, memos=False):
    if op[0] == "vote":
        return "%s: %s" % (op[1]["voter"], construct_identifier(op[1]["author"], op[1]["permlink"]))
    elif op[0] == "comment":
        return "%s: %s" % (op[1]["author"], construct_identifier(op[1]["author"], op[1]["permlink"]))
    elif op[0] == "transfer":
        str_ = "%s -> %s %s" % (
            op[1]["from"],
            op[1]["to"],
...
def print_permissions(account):
    ...
    for permission in ["owner", "active", "posting"]:
        ...
        t.add_row([permission, account[permission]["weight_threshold"], "\n".join(auths)])
    print(t)
...
def steemtailentry():
    ...
    parser.add_argument(
        '-j', '--json', help='Output as JSON instead of human-readable pretty-printed format', action='store_true'
    )

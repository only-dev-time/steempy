...
class Dex(object):
    ...
    def _get_asset(self, symbol):
        """Return the properties of the assets tradeable on the network.
    ...
    def buy(
        ...
        op = operations.LimitOrderCreate(
            **{
                ...
                "min_to_receive": '{:.{prec}f} {asset}'.format(amount, prec=quote["precision"], asset=quote["symbol"]),
                "fill_or_kill": killfill,
                "expiration": fmt_time_from_now(expiration),
            }
        )
    ...
    def sell(
        ...
        op = operations.LimitOrderCreate(
            **{
                ...
                "amount_to_sell": '{:.{prec}f} {asset}'.format(amount, prec=quote["precision"], asset=quote["symbol"]),

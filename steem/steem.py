...
class Steem:
    ...
    :param bool debug: Elevate logging level to `logging.DEBUG`.
        Defaults to `logging.INFO`.

    :param bool no_broadcast: If set to ``True``, committal actions like
        sending funds will have no effect (simulation only).

    Optional Arguments (kwargs):

    :param list keys: A list of wif keys. If provided, the Wallet will
        use these keys rather than the ones found in BIP38 encrypted
        wallet.

    :param bool unsigned: (Defaults to False) Use this for offline signing.

    :param int expiration: (Defualts to 60) Size of window in seconds
